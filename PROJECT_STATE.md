...
- **Automated test coverage is narrow** — Vitest (14 tests for `normalizeWsHost`) + pytest (31 tests for `IntegrityMonitor` + `_finalize_status`) cover the integrity-critical surfaces. Most of the codebase still relies on manual browser verification + root-level `test_e2e*.js` Playwright smoke flows. Run with `npm run test` (frontend) / `python -m pytest` (backend).
